...
if(isset($_GET['id'])) {
    ...
    if(isset($_SESSION['steamid'])) {
        ...
        if($utype == 'admin'){
            ...
            $sql = "SELECT * FROM `uploads` WHERE `id`='$query' && `description`!='d1z3j51f81'";
            $resultsa = mysqli_query($conn, $sql);
            while ($rowa = mysqli_fetch_assoc($resultsa)) {
                $upid = $rowa['id'];
                $upname = $rowa['name'];
...
<div class="gd">
    ...
    <hr>
    <div class="gd1">
        <div class="gd1img"><img src="<?php echo $upimages['logo']; ?>.jpg"></div>
        <div class="gd1img"><img src="<?php echo $upimages['gallery'][1]; ?>.jpg"></div>
    ...
    <hr>
    <div>Описание:<br><?php echo $updescription; ?></div>
    <hr>
    <div>Цена: <?php echo $upprice; ?></div>
    <hr>
    <div>Аддон: <a href="./assets/php/adminKO/download.php?id=<?php echo $upid; ?>">Скачать</a></div>
    <hr>
    <div><b onclick="udapr('1', '<?php echo $upid; ?>')">Одобрить</b> / <b onclick="udapr('2','<?php echo $upid; ?>')">Удалить</b></div>
</div>
