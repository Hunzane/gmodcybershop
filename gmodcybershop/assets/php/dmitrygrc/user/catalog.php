...
<div class="main">
    <div class="centrim">
        <div class="headersearchbargrid">
            <?php $query = $_POST['myData']; $query = json_decode($query, true); $query = $query[serachber]; ?>
            <?php if($query != '') { $searcher = htmlspecialchars($query, ENT_QUOTES); } ?>
...
<?php
    ...
    if($searcher != ''){
        $sql = "SELECT * FROM `products` WHERE `name` LIKE '$searcher%";
    } else {
        $sql = "SELECT * FROM `products` ORDER BY `ID` DESC LIMIT 25";
    }
